...
class Stopwatch extends React.Component {
  render() {
    return <div className='stopwatch'>{this.props.time}</div>;
  }
}
...
class App extends React.Component {
  ...
  format(time) {
    return `${pad0(time.minutes)}:${pad0(time.seconds)}:${pad0(
  ...
  result() {
    if (
      ...
      this.setState(() =>
        this.state.result.push(this.format(this.state.times))
      );
    }
  }
...
class Result extends React.Component {
  get times() {
    if (this.props.results.length) {
      return this.props.results.map((time, index) => (
        <li key={index}>{time}</li>
      ));
    }
  }
  render() {
    return <ul className='results'>{this.times}</ul>;
  }
}
